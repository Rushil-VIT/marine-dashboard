...
function RecommendationPanel({
  ...
  const recommendations = useMemo(
    ...
    [dataMode, selectedSpill, allSpills, selectedPollution, allPollutions]
  );

  const isVisible = embedded ? true : isOpen;

  const panelStyle = {
    position: embedded ? "relative" : "absolute",
    top: embedded ? "auto" : "72px",
    right: embedded ? "auto" : "16px",
    width: embedded ? "100%" : "360px",
    maxHeight: embedded ? "none" : "calc(100% - 96px)",
    ...
    transition: embedded ? "none" : "transform 0.25s ease, opacity 0.25s ease",
